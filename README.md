...
прогресс идет в полосе формы ниже. Ссылка на ассет редиректит на `objects.githubusercontent.com`,
и если браузер запретит этот запрос, в карточке появится подсказка — скачайте `.bin` вручную
по ссылке и залейте формой **«↑ OTA — Загрузка .bin»**. Ручной способ работает всегда.

> На **WT32-SC01 Plus** кнопки автоустановки нет: CI кладет в релиз бинарники только
> ESP32 DEVKIT1, и прошивка от другой платы устройство не оживит. Проверка версии при этом
