...
| `GET`  | `/metrics`        | Текущие значения датчиков и активный алерт для вкладки онлайн-монитора (JSON) |
| `GET`  | `/update`         | Страница OTA-обновления прошивки (редирект на `/`) |
| `POST` | `/update`         | Загрузить `.bin` файл прошивки (multipart) |

Главная страница разделена на две вкладки: **«Настройки»** с редактором конфигурации и
**«Онлайн-монитор»** с теми же параметрами, которые выводятся на TFT-дисплей в реальном времени.
