...
| **Пассивный бузер** | Пьезоизлучатель для звукового оповещения об алертах. Подключается к GPIO 26. |
...
| Метод | Путь | Описание |
|-------|------|---------|
| `GET`  | `/`               | Главная страница (HTML) — редактор конфига порогов |
...
| `GET`  | `/update`         | Страница OTA-обновления прошивки (HTML) |
...
Все пороги хранятся в файловой системе LittleFS (`/config.json`) и доступны для редактирования через веб-интерфейс. Кнопка **«По умолчанию»** сбрасывает конфиг к заводским значениям.
...
| Параметр | Знач. по умолч. | Описание |
|----------|----------------|---------|
| `red_low`   | 11.7 В | Ниже — красный (разряд) |
| `green_min` | 12.5 В | Начало зелёной зоны |
...
Для каждой проверки действует **антидребезг 15 секунд** — повторный сигнал по той же проверке не раньше, чем через 15 с.

### Коды алертов

| Код | Название | Описание | Параметры по умолч. |
|-----|---------|---------|-------------------|
| **E01** | Engine Oil Temp High | Температура масла ДВС превысила максимум | Max: 120 °C |
| **E02** | Engine Coolant Temp High | Температура ОЖ ДВС превысила максимум | Max: 103 °C |
| **E03** | Radiator Coolant Temp High | Температура ОЖ радиатора превысила максимум | Max: 95 °C |
| **E04** | CVT Oil Temp High | Температура масла вариатора превысила максимум | Max: 110 °C |
...
| **E08** | Oil Pressure Low | Напряжение датчика давления масла ниже нормы для текущих оборотов | RPM порог: 3000, Min low: 1.0 В, Min high: 1.5 В |
...
**Настройка проверок** — для каждой проверки (E01–E08) можно:
- Включить / отключить проверку (чекбокс).
