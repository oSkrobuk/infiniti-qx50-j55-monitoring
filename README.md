...
Соберите проект в PlatformIO (кнопка **Build** или `pio run`). После сборки в папке `.pio/build/esp32doit-devkit-v1/` появятся три файла:
...
1. Перейдите на сайт Espressif: https://www.espressif.com/en/support/download/other-tools
2. Скачайте **Flash Download Tools** (раздел «Flash Download Tools (ESP8266 & ESP32 & ESP32-S2)»).
