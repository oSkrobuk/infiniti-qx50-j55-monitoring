...
| VCC    | **3,3 В** | **3,3 В**, DEBUG контакт 2 | Питание |
...
> а не вход питания WT32, и не указывает допустимый ток внешней нагрузки. Его можно использовать для
> маломощного CAN-трансивера, но не для питания платы или других нагрузок с заметным током.
...
> Разъем `SPK+/SPK-` на WT32-SC01 Plus подключен к встроенному I2S-усилителю. Текущая прошивка
> управляет внешним пассивным бузером через GPIO 12 и не использует этот разъем.
...
1. Воспроизводит **звуковой сигнал** через бузер (GPIO 26 на ESP32 DEVKIT1, GPIO 12 на WT32-SC01 Plus).
