...
> На **WT32-SC01 Plus** кнопки автоустановки нет: `firmware_s3.bin` прикладывается к GitHub Release,
> но не публикуется в служебную ветку `firmware`. Проверка версии работает, а образ S3 можно скачать
> из релиза и установить вручную через форму OTA.
