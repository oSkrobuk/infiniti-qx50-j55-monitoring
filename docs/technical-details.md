...
|--------|----------------|----------------|----------|
| CTX    | GPIO **32** | GPIO **10**, EXT_IO контакт 3 | CAN TX (МК → трансивер) |
| CRX    | GPIO **34** | GPIO **11**, EXT_IO контакт 4 | CAN RX (трансивер → МК) |
| VCC    | **3,3 В** | Внешний стабилизатор **3,3 В** | Питание |
| GND    | **GND** | **GND**, EXT_IO контакт 2 | Земля |
| CANH   | OBD-II пин **6** | OBD-II пин **6** | Шина CAN High |
| CANL   | OBD-II пин **14** | OBD-II пин **14** | Шина CAN Low |

> На WT32-SC01 Plus GPIO 10 и GPIO 11 выведены на разъем EXT_IO и не конфликтуют со встроенными
> дисплеем и сенсорной панелью.

> На контакте 1 разъема EXT_IO находится 5 В — подключать к нему VCC модуля WVCMCU-230 нельзя.
> Вывод 3,3 В находится на контакте 2 разъема DEBUG, но производитель помечает его как опорный и не
> указывает допустимый ток внешней нагрузки. В этом проекте CAN-трансивер питается от отдельного
> стабилизатора 3,3 В. Питать его от DEBUG-контакта 2 или от 5 В разъема EXT_IO не рекомендуется.
...
|-------|----------------|----------------|----------|
| + / S | GPIO **26** | GPIO **12**, EXT_IO контакт 5 | Управляющий сигнал |
| - / GND | **GND** | **GND**, EXT_IO контакт 2 | Земля |
...
Скачайте последнюю версию релиза с GitHub. Для ESP32 DEVKIT1 нужны `bootloader.bin`,
`partitions.bin` и `firmware.bin`. Для WT32-SC01 Plus нужны `bootloader_s3.bin`,
`partitions_s3.bin` и `firmware_s3.bin`.
...
### 3.4 Настройка WT32-SC01 Plus

1. В стартовом окне выберите **ChipType:** `ESP32-S3`, **WorkMode:** `Develop` и нажмите **OK**.
2. В главном окне добавьте три строки:
...
2. Для WT32 зажмите **BOOT**, нажмите и отпустите **RESET**, затем отпустите **BOOT**.
3. Нажмите кнопку **ERASE** — дождитесь надписи `FINISH` (очистка памяти).
4. Нажмите кнопку **START** — начнётся прошивка, прогресс-бар покажет ход загрузки.
5. После надписи `FINISH` нажмите кнопку **EN** / **RESET** на плате или переподключите USB.
